...
class dataExtract:
            ...
            while k<endsearch:
                ...
                if (magphs[k]&self.outmem_fff_maskhi)>>16 == 0x5555:
                    ...
                    if True:
                        ...
                        self.event_length = (magphs[k]&self.outmem_fff_mask);
                        ...
                        datalongaaa = magphs[k+1]

                        datalong1 =magphs[k+2] 
                        ...
                        dp=(magphs[(k+3):(k+3+self.outmem_datalen)])&0xffff
                        dm=((magphs[(k+3):(k+3+self.outmem_datalen)])&0xffff0000)>>16
                        ...
                        print 'chan   %d  k  %d  is_pulse  %d  timestamp  %x  aaa 0x%x data1 0x%x meanphs %f '%\
                              (chan,k,is_pulse,timestamp,datalongaaa, datalong1, mean(dataph))
                        ...
                        nextk=k+self.outmem_headerlen + self.outmem_datalen+1        
    ...
    def getEventFromStream(self,chan=0,index=-1,ts=-1,events=-1):
        ...
        ed_=st_+events[chan]['event_len'][index]
...
#say we readback 20 bins. then we make time series of one of these bins.
    ...
    def plotEvents2D(self,fignum=5, chans=-1,data=-1,stevent=0,nevents=100):
